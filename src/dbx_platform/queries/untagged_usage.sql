-- Share of spend with no custom tags at all over the last :days days —
-- quantifies the cost-attribution gap. Per-required-tag checks on live
-- clusters/jobs are done API-side by `governance tag-compliance`.
