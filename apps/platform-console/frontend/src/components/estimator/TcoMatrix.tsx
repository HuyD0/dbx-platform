...
function summarizeScenario(
  ...
  const computeItems = estimate.line_items.filter(
    (item) => item.component === "serving_compute",
  );
...
export function TcoMatrix({
    ...
    return (
      <Card>
        ...
        <p className="text-sm text-muted">
          No operating tiers are available for this estimate.
        </p>
      </Card>
    );
  }
  ...
  const requestedScenario = SCENARIOS.includes(scenario as ScenarioKey)
    ...
    : "databricks";
  const activeSummary =
    summaries.find((summary) => summary.key === requestedScenario) ??
    summaries[0];
  const alternativeSummary = summaries.find(
    (summary) => summary.key !== activeSummary.key,
  );
  const safeBaselineHours = baselineHours > 0 ? baselineHours : BASELINE_HOURS;
  const missingPrices = [
    ...new Set(summaries.flatMap((summary) => summary.missingPrices)),
  ];
  const activeCheckingCost = ENVS.reduce(
    (sum, env) => sum + (activeSummary.byEnv[env]?.evalTax ?? 0),
    0,
  );
  const activeCheckingShare =
    activeSummary.total && activeSummary.total > 0
      ? activeCheckingCost / activeSummary.total
      : 0;
  ...
  const tableRows: {
  ...
  }[] = [
    {
      ...
      values: summaries.map((summary) => summaryValue(summary.total, usd)) as [
        string,
        string,
      ],
    },
    {
      ...
      values: summaries.map((summary) =>
        summary.total === null
          ...
          : usd(
              ENVS.reduce(
                (sum, env) => sum + (summary.byEnv[env]?.runCost ?? 0),
                0,
              ),
            ),
      ) as [string, string],
    },
    {
      ...
      values: summaries.map((summary) =>
        summary.total === null
          ...
          : usd(
              ENVS.reduce(
                (sum, env) => sum + (summary.byEnv[env]?.evalTax ?? 0),
                0,
              ),
            ),
      ) as [string, string],
    ...
    ...ENVS.map((env) => ({
      ...
      values: summaries.map((summary) => usd(summary.byEnv[env]?.total)) as [
        string,
        string,
      ],
    })),
    {
      ...
      values: summaries.map((summary) =>
        summaryValue(
          summary.total,
          (value) => `${usd(value / safeBaselineHours)} / h`,
        ),
      ) as [string, string],
    },
    {
      ...
      values: summaries.map((summary) => summary.computeAllocation) as [
        string,
        string,
      ],
    },
    {
      ...
      values: summaries.map((summary) =>
        summaryValue(summary.computeSpend, usd),
      ) as [string, string],
    },
    {
    ...
    <Card>
      ...
      >
        <div>
          ...
          {activeTier.rigor_locked && (
            <span className="mt-2 inline-block">
              <Badge tone="info">
                reviews fixed at {activeTier.default_rigor_pct}%
              </Badge>
            </span>
          )}
        </div>
        ...
        <div className="border-t border-grid pt-3 lg:border-l lg:border-t-0 lg:pl-4 lg:pt-0">
          ...
          <p className="mt-1 text-[11px] text-muted">
            Side-by-side planning alternative
          </p>
        </div>
        ...
        <div className="border-t border-grid pt-3 lg:border-l lg:border-t-0 lg:pl-4 lg:pt-0">
          ...
          <p className="mt-1 text-[11px] text-muted">
            Per month · explicit comparison basis
          </p>
        </div>
      </div>
      ...
      {missingPrices.length > 0 && (
        <div className="mt-3 flex flex-wrap items-center gap-2" role="status">
          <Badge tone="warning">
            {missingPrices.length} price(s) unavailable
          </Badge>
          <HelpTip label="About missing prices">
            Some meters had no price in the current snapshot, so these totals
            are incomplete — never silently zero. Refresh the pricing snapshot
            to fill the gaps.
          </HelpTip>
        </div>
      )}
      ...
      <div className="mt-3 flex flex-wrap items-center justify-between gap-2">
        <p className="text-xs text-muted">
          Expand for environment allocation, compute capacity, and inference
          assumptions.
        </p>
        <button
          type="button"
      ...
      {expanded && (
        <div id={detailsId} className="mt-4 overflow-x-auto">
          ...
          >
            ...
            <thead className="bg-warning-surface text-left text-xs text-brand-maroon">
              <tr>
                <th
                  scope="col"
                  className="rounded-l-lg border-y border-l border-grid px-3 py-3 font-semibold"
                >
              ...
              {tableRows.map((row) => (
                <tr key={row.label} className="align-top hover:bg-tint/50">
                  ...
                  {row.values.map((value, index) => (
                    <td
                      key={SCENARIOS[index]}
                      className="px-3 py-3 tabular-nums text-ink-2"
                    >
                      {value}
                    </td>
                  ))}
          ...
          {activeCheckingShare >= CHECKING_ANOMALY_SHARE && (
            <div
              className="mt-3 flex flex-wrap items-center gap-2"
              role="status"
            >
              <Badge tone="warning">
                checking is {Math.round(activeCheckingShare * 100)}% of selected
                spend
              </Badge>
              <HelpTip label="Why is this flagged?">
                Verifying the AI's work accounts for at least half of this
                scenario's total spend. Lower the review coverage or revisit the
                checking strategy if that is unexpected.
              </HelpTip>
            </div>
          )}
        </div>
